...
public class CreateRentalCommand : IRequest<CreatedRentalDto>, ILoggableRequest
{
    ...
    public class CreateRentalCommandHandler : IRequestHandler<CreateRentalCommand, CreatedRentalDto>
    {
        ...
        public CreateRentalCommandHandler(IRentalRepository rentalRepository, IMapper mapper,
                                          ...
                                          IAzureBusService<Rental> azureBusService
            )
        {
        ...
        public async Task<CreatedRentalDto> Handle(CreateRentalCommand request, CancellationToken cancellationToken)
        {
            ...
            _mailService.SendMail(new Mail
            {
                ...
                ToEmail = "mesutio@outlook.com",
                ToFullName = "Mesut io"
            });

            CreatedRentalDto createdRentalDto = _mapper.Map<CreatedRentalDto>(createdRental);
    //        var rentalCreatedEvent = new CreatedEventRental()
    //        {
    //            CreatedAt = DateTime.Now,
    //            Id = createdRentalDto.Id,
    //            CarModelName=createdRentalDto.CarModelName,
    //            CustomerMail=createdRentalDto.CustomerMail,
    //            RentStartDate=createdRentalDto.RentStartDate,
    //            RentEndDate=createdRentalDto.RentEndDate
               
    //};
    //        if (createdRentalDto != null)
    //        {
    //            await _azureBusService.CreateQueeIfNotExists(RentalMessages.RentalCreatedQuee);
    //            await _azureBusService.sendMessageToQuee(rentalCreatedEvent, RentalMessages.RentalDeletedQuee);
